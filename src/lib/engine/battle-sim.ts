...
function evaluateMoveOption(
  ...
  if (moveName === "Fake Out" && user.canFakeOut) {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      // Don't overvalue Fake Out into flinch-immune or secondary-blocking targets.
...
function executeMove(
  ...
  for (const t of targets) {
    ...
    // Secondary effects
    if (!blocksMoveSecondaryEffects(t, user.ability) && move.secondary && state.rng() * 100 < move.secondary.chance) {
